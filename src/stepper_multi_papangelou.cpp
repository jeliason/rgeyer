...
// [[Rcpp::export]]
NumericMatrix rstepper_multi_log_papangelou_grid_c(List theta1_list,
                                                   List theta2_list,
                                                   NumericMatrix from,
                                                   NumericMatrix to,
                                                   IntegerVector types,
  ...
  NumericMatrix log_papangelou(to.nrow(), ntypes);
  ...
  for(t = 0; t < ntypes; t++) {
    graphs1.at(t) = new Graphnested(&x, ranges1.at(t));
    maxr = maxv(ranges1.at(t));
    if(maxr > maxR) maxR = maxr;
    target.at(0) = types[t]; //
    graphs1.at(t)->set_pars_i(target);
    graphs1.at(t)->set_pair_suitable_method(&Graphnested::pair_suitable_same_mark_but_of_given_set);
    graphs1.at(t)->compute_edges();
  ...
  for(i = 0; i < to.nrow(); i++) {
    xn = to.at(i,0);
    yn = to.at(i,1);
    x.push_back(xn, yn, types[0]);
    // compute the potential neighbours
    neighs_of_last.clear();
    for(t=0; t< x.size();t++) if(x.getDist(&t, &last)<maxR) neighs_of_last.push_back(t);
    // add potential neighbours for each graph and update the links:
    for(t=0; t < ntypes; t++) {
      graphs1.at(t)->set_last_points_potential_neighbours(neighs_of_last);
      graphs1.at(t)->update_edges_after_addition_stored();
    }
    for(t=0; t < npairs; t++) {
      graphs2.at(t)->set_last_points_potential_neighbours(neighs_of_last);
      graphs2.at(t)->update_edges_after_addition_stored();
    }
    // then start changing the type. 0 needs to be here for some reason??
    for(ityp = 0;  ityp < ntypes; ityp++){
      // change the type
      mn = types[ityp];
      x.setType(&last, mn);
      // recompute the graphs:
      for(t=0; t < ntypes; t++) graphs1.at(t)->update_edges_after_mark_change();
      for(t=0; t < npairs; t++) graphs2.at(t)->update_edges_after_mark_change();
      // potential
      newpot = potential_multi(&x, theta0, &thetas1, &thetas2, &saturations1, &saturations2, &graphs1, &graphs2);
      // the difference is the log-pap
      log_papangelou.at(i, ityp) = newpot - pot;
    }
    // remove the point
    x.pop_back();
